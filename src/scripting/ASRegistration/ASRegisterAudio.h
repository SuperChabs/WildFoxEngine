...
inline void PlayOneShot(AudioSystem *audioSystem, const std::string &path, float volume, float pitch) {
    if (!audioSystem) return;
    audioSystem->PlayOneShot(path, volume, pitch);
}

inline void RegisterAudio(asIScriptEngine *engine, AudioSystem *audioSystem) {
